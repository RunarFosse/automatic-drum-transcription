...
\draw[arrow] (h) -- (0, -8.75) node[rectangle,
rounded corners,
draw,
anchor=north,
minimum size=1em,
...
\draw[arrow] (v) -- (0, -10.25) node[rectangle, 
rounded corners, 
draw, 
anchor=north, 
fill=blue!20,
...
\draw[arrow] (w) -- (0, -11.75);

\node[anchor=north, label=below:{Output Probabilities}] at (0, -11.75){\footnotesize{($T \times 5$)}};
...
\begin{scope}[on background layer]
    ...
    \begin{scope}[even odd rule]
        ...
        ($(pe.north east) + (0,0.25)$) rectangle ($(pe.south east) + (2,-0.25)$);
...
\draw[arrow] (p |- imnp.south) -- (7.5, -15) node[anchor=north] {\footnotesize{($T \times D_e$)}};
